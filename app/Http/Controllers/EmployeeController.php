...
class EmployeeController extends Controller
{
  ...
  public function index()
  {
    if (\Auth::user()->can('manage employee')) {
      if (Auth::user()->type == 'Employee') {
        ...
        $employees = Employee::where('created_by', \Auth::user()->creatorId())->with(['designation', 'branch', 'department', 'workShifts', 'employmentType'])->get();
      }
  ...
  public function store(Request $request)
  {
    if (\Auth::user()->can('create employee')) {
      ...
      // Prevent overlapping work shifts
      $selectedShiftIds = $request->work_shifts;
      $shifts = WorkShift::with('workShiftDays')->whereIn('id', $selectedShiftIds)->get();
      $windows = [];
      foreach ($shifts as $shift) {
        foreach ($shift->workShiftDays as $dayObj) {
          $day = $dayObj->day;
          $start = $shift->from;
          $end = $shift->to;
          $isCrossMidnight = $end < $start;
          if ($isCrossMidnight) {
            // Window 1: from start to 23:59:59 on the same day
            $windows[] = [
              'shift_id' => $shift->id,
              'day' => $day,
              'start' => $start,
            ...
            // Window 2: from 00:00:00 to end on the next day
            $nextDay = ($day + 1) % 7; // 0=Sunday, 6=Saturday
            $windows[] = [
              'shift_id' => $shift->id,
              'day' => $nextDay,
              'start' => '00:00:00',
        ...
        for ($j = $i + 1; $j < count($windows); $j++) {
          ...
          if ($a['day'] == $b['day']) {
            ...
            if ($aStart < $bEnd && $bStart < $aEnd) {
              return redirect()->back()->withInput()->with('error', __('Selected work shifts overlap on at least one day. Please choose non-overlapping shifts.'));
            }
          }
        }
      }
      ...
      $employee->workShifts()->sync($request->work_shifts);
  ...
  public function edit($id)
  {
    ...
    if (\Auth::user()->can('edit employee')) {
      ...
      $selectedWorkShifts = $employee->workShifts->pluck('id')->toArray();
      return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents', 'departmentData', 'managers', 'workShifts', 'employmentTypes', 'selectedWorkShifts'));
    } else {
      return redirect()->back()->with('error', __('Permission denied.'));
    }
  }
