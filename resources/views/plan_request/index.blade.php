                    ...
                    <table class="table table-striped table-hover align-middle datatable">
                        ...
                        @if($plan_requests->count() > 0)
                            @foreach($plan_requests as $prequest)
                                <tr>
                                    <td><div class="font-style">{{ $prequest->user->name }}</div></td>
                                    <td><div class="font-style">{{ $prequest->user->phone }}</div></td>
                                    <td><div class="font-style">{{ $prequest->plan->name }}</div></td>
                                    <td><div>{{ $prequest->plan->max_users }}</div></td>
                                    <td><div>{{ $prequest->plan->max_customers }}</div></td>
                                    <td><div>{{ $prequest->plan->max_venders }}</div></td>
                                    <td><div>{{ $prequest->plan->max_clients }}</div></td>
                                    <td>
                                        @if($prequest->duration == "year")
                                            <span class="badge bg-info">{{ __('Yearly') }}</span>
                                    ...
                                    <td><span class="text-muted">{{ Utility::getDateFormated($prequest->created_at,true) }}</span></td>
                                    <td>
